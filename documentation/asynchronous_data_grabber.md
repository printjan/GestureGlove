...
1. The ingestion of serial data, synchronization of streams, window preprocessing, scaling, neural network forward pass, and shortcut dispatching are all executed sequentially in a single main-thread loop.
...
flowchart TD
    ...
    subgraph Data Grabber (I/O & Preprocessing)
        ...
        DG_Thread -->|2. process_window| Preprocess[Calibration, Filters & Orientation]
        DG_Thread -->|3. transform_fn| Transform[Slice Channels & TimeSeriesScaler]
    ...
    subgraph Inference System (Compute Bound)
        ...
        Main_Thread -->|4. predict| CNN[Multi-Branch 1D CNN Model]
        Main_Thread -->|5. feed| Dispatcher[GestureDispatcher]
        Dispatcher -->|6. trigger| Controller[PowerPointController / Keyboard OS Event]
    end
```
...
2.  **Grabber Loop (Producer Part 2)**: The `AsynchronousDataGrabber` background thread runs at a high frequency (e.g. 100 Hz, `poll_interval_s=0.01`). It drains the queues, updates its internal sliding windows, aligns the dual IMU streams, extracts features, and runs a custom model-specific tensor transformation callback. It writes the result into a shared slot.
...
# 4. Instantiate and start Grabber
